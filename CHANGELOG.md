...
### branch: raw_RTF
    ...
    ++ assumption that original filtersettings where higher clocked to
       mitigate a fixed offset in the sampling (i.e. 0.04ms error, too long)
    ++ change accordingly the filter settings:
...
### branch: raw_w/oRTF
    01/29/2021
    -----------
    v3.5 (SHA: d8965cf8800d6359858577950bda478707f865dd)
    ++ archive upload and reduced (no more power, adjusted, rt_prad, single_channel)
    ++ removed analogue voltage ouput from VI completely,
       no hardware interface, no calculation
